...
const FLOWS: Flow[] = [
  {
    ...
    accent: '#CF1134',
    nodes: [
      { x: 2, y: 22, label: 'ICP search', icon: '🔎', c: '#5B4BA6' },
      { x: 30, y: 6, label: 'AI research', icon: '🤖', c: '#CF1134' },
      { x: 30, y: 38, label: 'Personalise', icon: '✍️', c: '#B8611B' },
  ...
  },
  {
    ...
    accent: '#5B4BA6',
    nodes: [
      { x: 2, y: 22, label: 'One idea', icon: '💡', c: '#A97B10' },
      { x: 30, y: 22, label: 'AI drafts', icon: '🤖', c: '#CF1134' },
      { x: 58, y: 4, label: 'Instagram', icon: '📸', c: '#CF1134' },
      { x: 58, y: 24, label: 'LinkedIn', icon: '💼', c: '#5B4BA6' },
      { x: 58, y: 44, label: 'Blog + SEO', icon: '📝', c: '#1F7A3D' },
      { x: 84, y: 24, label: 'Metrics loop', icon: '📈', c: '#B8611B' },
    ],
  ...
  },
  {
    ...
    accent: '#1F7A3D',
    nodes: [
      { x: 2, y: 22, label: 'Enquiry in', icon: '💬', c: '#1F7A3D' },
      { x: 30, y: 6, label: 'AI qualifies', icon: '🤖', c: '#CF1134' },
      { x: 30, y: 38, label: 'CRM entry', icon: '📊', c: '#5B4BA6' },
      { x: 58, y: 22, label: 'Slot offered', icon: '📅', c: '#B8611B' },
      { x: 84, y: 22, label: 'Call booked', icon: '✅', c: '#A97B10' },
    ],
    wires: ['M 16 26 C 24 26, 22 11, 30 11', 'M 16 28 C 24 28, 22 43, 30 43', 'M 44 11 C 52 11, 50 26, 58 26', 'M 44 43 C 52 43, 50 28, 58 28', 'M 72 27 L 84 27'],
  },
  {
    ...
    nodes: [
      ...
      { x: 2, y: 38, label: 'Bank/API', icon: '🏦', c: '#5B4BA6' },
      { x: 30, y: 22, label: 'Reconcile', icon: '🔁', c: '#B8611B' },
      { x: 58, y: 22, label: 'AI narrates', icon: '🤖', c: '#CF1134' },
      { x: 84, y: 22, label: '9am report', icon: '☕', c: '#A97B10' },
    ],
    ...
    <svg viewBox="0 0 100 56" className="w-full" role="img" aria-label={`${f.title} workflow`}>
      ...
      {f.nodes.map((n, i) => (
        <g key={n.label} className="wf-node" style={{ animationDelay: `${i * 0.45}s` }}>
          ...
          <text x={n.x + 7} y={n.y + 8.4} textAnchor="middle" fontSize="2" fill="#16130E" fontFamily="system-ui" fontWeight="600">{n.label}</text>
        </g>
      ))}
    </svg>
  )
